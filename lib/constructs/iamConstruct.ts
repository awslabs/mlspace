...
export class IAMConstruct extends Construct {
    ...
    constructor (scope: Stack, id: string, props: IAMStackProp) {
        super(scope, id);
        ...
        const ec2ArnBase = `arn:${scope.partition}:ec2:${Aws.REGION}:${scope.account}`;
        ...
        const notebookPolicyStatements = (partition: string, region: string, allow_all_instances: boolean = false) => {
            const statements = [
                ...
                new PolicyStatement({
                    ...
                    resources: [
                        `arn:${partition}:sagemaker:${region}:${scope.account}:endpoint-config/*`,
                    ],
                }),
                // Model Permissions
                new PolicyStatement({
                    ...
                    resources: [`arn:${partition}:sagemaker:${region}:${scope.account}:model/*`],
                    conditions: {
                        Null: {
            ...
            if (props.enableTranslate) {
                ...
                statements.push(new PolicyStatement({
                    ...
                    resources: [
                        `arn:${partition}:iam::${scope.account}:role/${props.mlspaceConfig.IAM_RESOURCE_PREFIX}*`,
                    ],
                    conditions: {
                        StringEquals: {
            ...
            if (!props.mlspaceConfig.MANAGE_IAM_ROLES || allow_all_instances) {
                statements.push(
                    ...
                    new PolicyStatement({
                        ...
                        resources: [
                            `arn:${partition}:sagemaker:${region}:${scope.account}:*`,
                        ],
                        conditions: {
                            Null: {
                                ...requestTagsConditions,
                ...
                statements.push(
                    ...
                    new PolicyStatement({
                        ...
                        resources: [
                            `arn:${partition}:sagemaker:${region}:${scope.account}:training-job/*`,
                            `arn:${partition}:sagemaker:${region}:${scope.account}:hyper-parameter-tuning-job/*`
                        ],
                        conditions: {
                            Null: {
                                'sagemaker:VpcSecurityGroupIds': 'false',
        ...
        const instanceConstraintPolicyStatement = (partition: string, region: string, actionResourcePair: {[key: string]: string}) => {
            const [actions, resources] = Object.entries(actionResourcePair).reduce(([actionAccumulator, resourcesAccumulator], [action, resource]) => {
                return [[...actionAccumulator, `sagemaker:${action}`], [...resourcesAccumulator, `arn:${partition}:sagemaker:${region}:${scope.account}:${resource}/*`]];
            }, [[] as string[], [] as string[]]);
            ...
            if (props.mlspaceConfig.JOB_INSTANCE_CONSTRAINT_POLICY_ARN) {
                ...
                this.mlspaceJobInstanceConstraintPolicy = new ManagedPolicy(scope, 'mlspace-job-instance-constraint', {
                    ...
                    statements: [
                        ...
                        instanceConstraintPolicyStatement(scope.partition, Aws.REGION, {CreateTransformJob: 'transform-job'})[0]
                    ]
                });
            }
                    ...
                    'mlspace-project-user-role-boundary',
                    {
                        ...
                        statements: [
                            ...
                            new PolicyStatement({
                                ...
                                */
                                resources: [`arn:*:iam::${scope.account}:role/${props.mlspaceConfig.IAM_RESOURCE_PREFIX}*`],
                                conditions: {
                                    StringEquals: {
            ...
            const statements = [
                ...
                new PolicyStatement({
                    ...
                    resources: [
                         ...
                         */
                        `arn:${scope.partition}:iam::${scope.account}:role/EMR_DefaultRole`,
                        `arn:${scope.partition}:iam::${scope.account}:role/EMR_EC2_DefaultRole`,
                        `arn:${scope.partition}:iam::${scope.account}:role/${roleName}`,
                    ],
                }),
                ...
                new PolicyStatement({
                    ...
                    resources: [`arn:${scope.partition}:s3:::*`],
                }),
                /**
                 * Notebook Permissions
                ...
                new PolicyStatement({
                    ...
                    resources: [
                        `arn:${scope.partition}:sagemaker:${Aws.REGION}:${scope.account}:notebook-instance/*`,
                    ],
                    conditions: {
                        StringEquals: {
                ...
                new PolicyStatement({
                    ...
                    resources: [
                        `arn:${scope.partition}:sagemaker:${Aws.REGION}:${scope.account}:notebook-instance-lifecycle-config/*`,
                    ],
                }),
                // Notebook Permissions
                new PolicyStatement({
                ...
                new PolicyStatement({
                    ...
                    resources: [
                        `arn:${scope.partition}:lambda:${Aws.REGION}:${scope.account}:function:mls-lambda-*`,
                    ],
                }),
                /**
                 * EMR Permissions
                ...
                new PolicyStatement({
                    ...
                    resources: [
                        `arn:${scope.partition}:elasticmapreduce:${Aws.REGION}:${scope.account}:cluster/*`,
                    ],
                }),
            ];
            ...
            if (props.mlspaceConfig.MANAGE_IAM_ROLES && this.mlSpacePermissionsBoundary) {
                ...
                statements.push(
                    new PolicyStatement({
                        ...
                        resources: [
                            `arn:${scope.partition}:iam::${scope.account}:role/${props.mlspaceConfig.IAM_RESOURCE_PREFIX}*`,
                        ],
                        conditions: {
                            StringEqualsIgnoreCase: {
                    ...
                    new PolicyStatement({
                        ...
                        resources: [
                            ...
                            `arn:${scope.partition}:iam::${scope.account}:role/${mlSpaceAppRoleName}`,
                            `arn:${scope.partition}:iam::${scope.account}:role/${mlSpaceNotebookRoleName}`,
                            `arn:${scope.partition}:iam::${scope.account}:role/${props.mlspaceConfig.IAM_RESOURCE_PREFIX}*`,
                        ],
                        conditions: {
                            StringEqualsIgnoreCase: {
                                // Only allow dynamic attachment for the deny services policy
                                'iam:PolicyARN': `arn:${scope.partition}:iam::${scope.account}:policy/${props.mlspaceConfig.IAM_RESOURCE_PREFIX}-app-denied-services`,
                                'iam:ResourceTag/system': props.mlspaceConfig.SYSTEM_TAG,
                            },
                        },
                    }),
                    ...
                    new PolicyStatement({
                        ...
                        // This needs to match the IAM_RESOURCE_PREFIX prefix in iam_manager.py
                        resources: [
                            `arn:${scope.partition}:iam::${scope.account}:policy/${props.mlspaceConfig.IAM_RESOURCE_PREFIX}*`,
                        ],
                    }),
                    new PolicyStatement({
                        effect: Effect.ALLOW,
            ...
            const s3WebsiteReadOnlyPolicy = new ManagedPolicy(scope, 'mlspace-website-read-policy', {
                statements: [
                    new PolicyStatement({
                        ...
                        resources: [`arn:${scope.partition}:s3:::${props.websiteBucketName}/*`],
                    }),
                ],
            });
