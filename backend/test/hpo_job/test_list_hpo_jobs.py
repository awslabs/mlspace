...
# Testing for the list_hpo_jobs Lambda function.
...
with mock.patch.dict("os.environ", TEST_ENV_CONFIG, clear=True):
    from ml_space_lambda.hpo_job.lambda_functions import list_resources as lambda_handler

project_name = "example_hpo_project_name"
...
def _mock_job_metadata(
    ...
    return ResourceMetadataModel(
        ...
        project_name,
        {
            "HyperParameterTuningJobArn": f"arn:aws:us-east-1:sagemaker:training-job/{identifier}",
...
def test_list_hpo_jobs_success(mock_resource_metadata_dao):
    mock_jobs_metadata = [
...
@mock.patch("ml_space_lambda.hpo_job.lambda_functions.resource_metadata_dao")
def test_list_hpo_jobs_client_error(mock_resource_metadata_dao):
    error_msg = {
...
@mock.patch("ml_space_lambda.hpo_job.lambda_functions.resource_metadata_dao")
def test_list_hpo_jobs_missing_parameters(mock_resource_metadata_dao):
