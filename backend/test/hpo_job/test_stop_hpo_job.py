...
# Testing for the stop_hpo_job Lambda function.
...
with mock.patch.dict("os.environ", TEST_ENV_CONFIG, clear=True):
    from ml_space_lambda.hpo_job.lambda_functions import stop as lambda_handler
...
def test_stop_hpo_job_success(mock_sagemaker):
    expected_response = generate_html_response(200, "Successfully stopped " + hpo_job_name)
...
@mock.patch("ml_space_lambda.hpo_job.lambda_functions.sagemaker")
def test_stop_hpo_job_client_error(mock_sagemaker):
    error_msg = {
...
@mock.patch("ml_space_lambda.hpo_job.lambda_functions.sagemaker")
def test_stop_hpo_job_missing_parameters(mock_sagemaker):
    expected_response = generate_html_response(400, "Missing event parameter: 'pathParameters'")
    assert lambda_handler({}, mock_context) == expected_response
    mock_sagemaker.stop_hyper_parameter_tuning_job.assert_not_called()
