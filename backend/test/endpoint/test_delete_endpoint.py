...
with mock.patch.dict("os.environ", TEST_ENV_CONFIG, clear=True):
    from ml_space_lambda.endpoint.lambda_functions import delete as lambda_handler
...
def test_delete_endpoint_success(mock_sagemaker, mock_resource_dao):
...
@mock.patch("ml_space_lambda.endpoint.lambda_functions.resource_scheduler_dao")
@mock.patch("ml_space_lambda.endpoint.lambda_functions.sagemaker")
def test_delete_endpoint_client_error(mock_sagemaker, mock_resource_dao):
    mock_event = {"pathParameters": {"endpointName": mock_endpoint_name}}
    ...
    mock_sagemaker.delete_endpoint.side_effect = ClientError(error_msg, "DeleteEndpoint")
    expected_response = generate_html_response(
        400,
        "An error occurred (MissingParameter) when calling the DeleteEndpoint operation: Dummy error message.",
    )
    assert lambda_handler(mock_event, mock_context) == expected_response
    mock_sagemaker.delete_endpoint.assert_called_with(EndpointName="example_endpoint")
    mock_resource_dao.delete.assert_not_called()


def test_delete_endpoint_generic_error():
