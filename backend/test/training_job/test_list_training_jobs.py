...
# Testing for the list_training_jobs Lambda function.
...
with mock.patch.dict("os.environ", TEST_ENV_CONFIG, clear=True):
    from ml_space_lambda.training_job.lambda_functions import list_resources as lambda_handler

mock_project_name = "example_project_name"
...
def _mock_job_metadata(
    ...
    return ResourceMetadataModel(
        ...
        mock_project_name,
        {
            "TrainingJobArn": f"arn:aws:us-east-1:sagemaker:training-job/{identifier}",
            ...
            "TrainingEndTime": "2023-10-04 13:46:53.326000+00:00" if status == "Completed" else None,
        },
    )


@mock.patch("ml_space_lambda.training_job.lambda_functions.resource_metadata_dao")
def test_list_training_jobs_success(mock_resource_metadata_dao):
    mock_jobs_metadata = [
...
@mock.patch("ml_space_lambda.training_job.lambda_functions.resource_metadata_dao")
def test_list_training_jobs_success_completed_jobs(mock_resource_metadata_dao):
    mock_jobs_metadata = [
        _mock_job_metadata("job-1"),
        _mock_job_metadata("job-2"),
    ]
    mock_resource_metadata_dao.get_all_for_project_by_type.return_value = PagedMetadataResults(
        mock_jobs_metadata,
        "fakeToken",
        ...
        lambda_handler(
            {
                ...
                "queryStringParameters": {"resourceStatus": "Completed", "nextToken": "abc123"},
            },
            mock_context,
        )
        == expected_response
    ...
    mock_resource_metadata_dao.get_all_for_project_by_type.assert_called_with(
        ...
        limit=100,
        next_token="abc123",
        filter_expression="metadata.TrainingJobStatus IN (:s0)",
        filter_values={":s0": "Completed"},
    )


@mock.patch("ml_space_lambda.training_job.lambda_functions.resource_metadata_dao")
def test_list_training_jobs_client_error(mock_resource_metadata_dao):
    error_msg = {
...
@mock.patch("ml_space_lambda.training_job.lambda_functions.resource_metadata_dao")
def test_list_training_jobs_missing_parameters(mock_resource_metadata_dao):
