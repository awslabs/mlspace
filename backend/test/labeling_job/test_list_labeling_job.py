...
with mock.patch.dict("os.environ", TEST_ENV_CONFIG, clear=True):
    from ml_space_lambda.labeling_job.lambda_functions import list_resources as lambda_handler

mock_context = mock.Mock()


def _mock_labeling_job_metadata(identifier: str, username: Optional[str] = MOCK_USERNAME) -> ResourceMetadataModel:
    return ResourceMetadataModel(
        ...
        MOCK_PROJECT_NAME,
        {
            ...
            "LabelingJobArn": f"arn:aws:us-east-1:sagemaker:labeling-job/{identifier}",
            "LabelingJobStatus": "InProgress",
            "LastModifiedTime": "2023-08-11 09:33:05.109000+00:00",
            "TaskType": "Image Classification (Single Label)",
        },
    )


@mock.patch("ml_space_lambda.labeling_job.lambda_functions.resource_metadata_dao")
def test_list_labeling_jobs_success(mock_resource_metadata_dao):
    mock_event = {
    ...
    mock_resource_metadata_dao.get_all_for_project_by_type.return_value = PagedMetadataResults(
        [
            _mock_labeling_job_metadata("job1"),
            _mock_labeling_job_metadata("job2"),
            _mock_labeling_job_metadata("job3"),
        ],
    )
    ...
    expected_response = generate_html_response(
        ...
        {
            "records": [
                _mock_labeling_job_metadata("job1").to_dict(),
                _mock_labeling_job_metadata("job2").to_dict(),
                _mock_labeling_job_metadata("job3").to_dict(),
            ],
        },
    )
    ...
    mock_resource_metadata_dao.get_all_for_project_by_type.assert_called_with(
        MOCK_PROJECT_NAME, ResourceType.LABELING_JOB, limit=10, next_token=None
    )


@mock.patch("ml_space_lambda.labeling_job.lambda_functions.resource_metadata_dao")
def test_list_labeling_jobs_client_error(mock_resource_metadata_dao):
    ...
    mock_resource_metadata_dao.get_all_for_project_by_type.assert_called_with(
        MOCK_PROJECT_NAME, ResourceType.LABELING_JOB, limit=100, next_token=None
    )


@mock.patch("ml_space_lambda.labeling_job.lambda_functions.resource_metadata_dao")
def test_list_labeling_jobs_generic_error(mock_resource_metadata_dao):
