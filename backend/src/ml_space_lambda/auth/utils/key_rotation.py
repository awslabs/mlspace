...
# def cleanup_old_secret_versions(secret_arn: str, keep_versions: int = 3) -> Dict:
#     """
#     Clean up old AWS Secrets Manager versions, keeping only the most recent ones.

#     This function deletes old secret versions (not just our internal key versions)
#     to prevent accumulation of versions in Secrets Manager.
...
#         version_info = response.get("VersionIdsToStages", {})

#         # Sort versions by creation date (most recent first)
#         # We'll keep versions that have AWSCURRENT or AWSPENDING labels
#         protected_versions = set()
#         version_list = []

#         for version_id, stages in version_info.items():
#             if SecretsManagerStage.CURRENT in stages or SecretsManagerStage.PENDING in stages:
#                 protected_versions.add(version_id)
#             else:
#                 # Get version details to sort by date
#                 try:
#                     version_response = secrets_client.get_secret_value(
#                         SecretId=secret_arn,
#                         VersionId=version_id
#                     )
#                     created_date = version_response.get("CreatedDate")
#                     if created_date:
#                         version_list.append((version_id, created_date))
#                 except Exception:
...
#         versions_to_keep = keep_versions - len(protected_versions)
#         versions_to_delete = []

#         if versions_to_keep > 0:
#             # Keep the most recent non-protected versions
#             for i, (version_id, _) in enumerate(version_list):
#                 if i >= versions_to_keep:
#                     versions_to_delete.append(version_id)
#         else:
#             # Delete all non-protected versions
#             versions_to_delete = [v[0] for v in version_list]
...
#                     SecretId=secret_arn,
#                     VersionStage=SecretsManagerStage.PENDING,  # Dummy stage
#                     MoveToVersionId=version_id,
#                     RemoveFromVersionId=version_id
#                 )
#                 deleted_versions.append(version_id)
#                 logger.info(f"Deleted secret version: {version_id}")
#             except Exception as e:
#                 logger.warning(f"Failed to delete version {version_id}: {e}")

#         return {
#             "success": True,
#             "deleted_versions": deleted_versions,
#             "protected_versions": list(protected_versions),
#             "message": f"Deleted {len(deleted_versions)} old secret versions"
#         }

#     except Exception as e:
#         logger.error(f"Failed to cleanup secret versions: {e}")
#         return {
#             "success": False,
#             "error": str(e)
#         }
...
def state_key_rotation_handler(event, context):
    ...
    try:
        ...
        if action == "initialize":
            ...
            result = get_key_status(secret_arn)
            # Convert Pydantic model to dict for JSON serialization
