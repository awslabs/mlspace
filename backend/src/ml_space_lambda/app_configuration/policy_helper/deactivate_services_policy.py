...
def update_activated_services_policy(
    app_configuration: AppConfigurationModel, iam_manager: IAMManager = None
) -> list[ResourceType]:
    """
    ...
    for service in enabled_services_dict.keys():
        ...
        if not enabled_services_dict[service] and service in SERVICE_DEACTIVATE_PROPERTIES:
            if "ResourceType" in SERVICE_DEACTIVATE_PROPERTIES[service]:
                resource_types_to_suspend.append(SERVICE_DEACTIVATE_PROPERTIES[service]["ResourceType"])
            if "Statements" in SERVICE_DEACTIVATE_PROPERTIES[service]:
                deny_policy_statements.extend(SERVICE_DEACTIVATE_PROPERTIES[service]["Statements"])

    # Check for permissions associated with multiple services
    for group in SERVICE_GROUP_DEACTIVATE_PROPERTIES:
    ...
    if len(deny_policy_statements) == 0:
        deny_policy_statements.extend(FILTER_DENY_STATEMENTS)
