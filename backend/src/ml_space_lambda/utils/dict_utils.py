...
def rename_dict_keys(input: dict, nameMappings: dict):
    """
    ...
    Args:
        ...
        nameMappings (int): A dict of old key names to new key names

    Returns:
        dict: A copy of input with keys possibly renamed
    """
    return {nameMappings.get(key, key): input.get(key) for key in input}
