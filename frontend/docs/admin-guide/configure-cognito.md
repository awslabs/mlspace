...
The deprecated `OIDC_URL` and `OIDC_CLIENT_NAME` parameters are no longer supported. You must use the new `AUTH_OIDC_URL` and `AUTH_OIDC_CLIENT_ID` parameters instead. See the [Enhanced Authentication Configuration Guide](./bff-authentication.md) for complete details on all AUTH_* parameters.
:::
