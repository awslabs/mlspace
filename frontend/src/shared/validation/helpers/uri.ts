...
ZodString.prototype.datasetUri = function (message?: errorUtil.ErrMessage): z.ZodString {
    ...
    return z.string().regex(/^s3:\/\/(?<bucket>[a-z0-9][a-z0-9.-]{1,61}[a-z0-9])\/(?<type>(global|group)|(private|project)\/(?<scope>.+))\/datasets\/(?<name>.+)\/(?<location>(?<prefix>(.+?\/)+)?(?<object>.+)?)$/, message || defaultMessage);
};
...
ZodString.prototype.datasetPrefix = function (message?: errorUtil.ErrMessage): z.ZodString {
    ...
    return z.string().regex(/^s3:\/\/(?<bucket>[a-z0-9][a-z0-9.-]{1,61}[a-z0-9])\/(?<type>(global|group)|(private|project)\/(?<scope>.+))\/datasets\/(?<name>.+)\/(?<location>(?<prefix>(.+?\/)+)?)$/, message || defaultMessage);
};
